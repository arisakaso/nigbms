...
def create_vector_block(L: list[Form]) -> PETSc.Vec:
    ...
    """
    maps = [(form.function_spaces[0].dofmap.index_map, form.function_spaces[0].dofmap.index_map_bs) for form in L]
...
def assemble_vector(L: typing.Any, constants=None, coeffs=None) -> PETSc.Vec:
    ...
    """
    b = create_petsc_vector(L.function_spaces[0].dofmap.index_map, L.function_spaces[0].dofmap.index_map_bs)
...
def _assemble_vector_nest_vec(b: PETSc.Vec, L: list[Form], constants=None, coeffs=None) -> PETSc.Vec:
    ...
    for b_sub, L_sub, const, coeff in zip(b.getNestSubVecs(), L, constants, coeffs, strict=False):
        with b_sub.localForm() as b_local:
...
def assemble_vector_block(
    ...
    with b.localForm() as b_local:
        b_local.set(0.0)
    return _assemble_vector_block_vec(b, L, a, bcs, x0, scale, constants_L, coeffs_L, constants_a, coeffs_a)
...
def _assemble_vector_block_vec(
    ...
    coeffs_a = (
        [[{} if form is None else _pack_coefficients(form._cpp_object) for form in forms] for forms in a]
        if coeffs_a is None
        else coeffs_a
    )
    ...
    for b_sub, L_sub, a_sub, const_L, coeff_L, const_a, coeff_a in zip(
        b_local, L, a, constants_L, coeffs_L, constants_a, coeffs_a, strict=False
    ):
    ...
    for submap, bc, _x0 in zip(maps, bcs0, x0_sub, strict=False):
...
def _assemble_matrix_nest_mat(
    ...
    for i, (a_row, const_row, coeff_row) in enumerate(zip(a, constants, coeffs, strict=False)):
        for j, (a_block, const, coeff) in enumerate(zip(a_row, const_row, coeff_row, strict=False)):
...
def apply_lifting_nest(
    ...
    for b_sub, a_sub, const, coeff in zip(b.getNestSubVecs(), a, constants, coeffs, strict=False):
        apply_lifting(b_sub, a_sub, bcs1, x0, scale, const, coeff)
    return b


def set_bc(b: PETSc.Vec, bcs: list[DirichletBC], x0: typing.Optional[PETSc.Vec] = None, scale: float = 1.0) -> None:
...
def set_bc_nest(
    ...
    for b_sub, bc, x_sub in zip(_b, bcs, x0, strict=False):
